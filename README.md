...
> **v0.3.8：修掉「资讯模型未返回有效 JSON」——模型把结果包进代码围栏时整期不投递的问题。** 现在会剥掉围栏再按原样严格校验，结构、来源与日期一律不放宽；提示词也明确要求不要围栏。
...
版本 **0.3.8**。仓库：https://github.com/iamhej/dsh-claudia 。面向单用户本机，macOS 优先；与 Today、DeepSeek 无官方隶属或背书关系。
